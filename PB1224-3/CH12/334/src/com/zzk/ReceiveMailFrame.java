...
public class ReceiveMailFrame extends JFrame {
    ...
    public void init() throws Exception {
        ...
        session = Session.getDefaultInstance(props,check);// 獲得Session對像
        store = session.getStore(receiveProtocol);// 獲得Store對像
        store.connect(receiveHost, null, null);// 連接接收服務器，並進行身份驗證
    }
    
    public void receiveMessage() throws Exception {
        Folder folder = store.getFolder("inbox");// 獲得inbox郵件夾
        if (folder == null) {
