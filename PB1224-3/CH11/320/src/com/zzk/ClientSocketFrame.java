    ...
    private void connect() { // 連接套接字方法
        ...
        try { // 捕捉例外
            ...
            while (true) {
                ...
                reader = new BufferedReader(new InputStreamReader(socket.getInputStream())); // 實例化BufferedReader對像
                index = Integer.parseInt(reader.readLine());// 獲得客戶登入服務器的索引值
                ta_info.append("你是第"+(index+1)+"個完成連接的使用者。\n"); // 純文字域中提示資訊
                getServerInfo();
            }
        } catch (Exception e) {
    ...
    public ClientSocketFrame() {
        ...
        addWindowListener(new WindowAdapter() {
            public void windowClosing(final WindowEvent e) {
                writer.println(String.valueOf(index));// 向服務器端發送退出客戶的索引值
            }
        });
        setTitle("客戶端程式");
